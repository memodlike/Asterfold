...
Asterfold 3.0.0 is local-first. Workspace data is stored in the local Chrome profile. The default release has no application backend, account, analytics, advertising, telemetry, cloud synchronization, host permissions or content scripts, and it makes no application network requests.
