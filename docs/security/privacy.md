...
- Pages, Boards, bookmarks, titles, saved URLs, descriptions, local custom-icon data retained for compatibility, and ordering;
...
The default build has no host permission and no configured application backend. Opening a saved URL is an explicit browser navigation. Favicons are obtained through Chrome's own `_favicon` resource; Asterfold does not render or newly persist the remote `favIconUrl` reported by a tab or an older backup. A custom icon is renderable only when it is a bounded local PNG/JPEG/WebP data asset; remote URLs and SVG are rejected. Optional cloud code is disabled unless the owner supplies a Supabase HTTPS origin and publishable key and rebuilds the extension.
...
- Privacy Mode removes bookmark titles from rendered text, tooltip/accessibility names and bookmark context-menu labels, disables clipboard actions, and does not construct the local search index. It is shoulder-surfing protection, not database encryption: IndexedDB records remain readable to the browser profile.
