...
Asterfold 3.5.3 uses Manifest V3, `host_permissions: []`, no content scripts and no remotely hosted executable code.
...
Asterfold 3.5.3 does not request `favicon`, `activeTab`, `alarms`, or `contextMenus`. It performs zero active-tab queries, zero background polling alarms, zero context menus, and makes zero third-party or internal `_favicon` requests. Local vector icons are used for visual fallbacks without requiring store permission justifications.
