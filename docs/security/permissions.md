...
Asterfold 3.5.1 uses Manifest V3, `host_permissions: []`, no content scripts and no remotely hosted executable code.
...
Asterfold 3.5.1 does not request `activeTab`, `alarms`, or `contextMenus`. It performs zero active-tab queries, zero background polling alarms, zero context menus, and uses `_favicon` only for a saved, safe HTTP(S) URL. Privacy Mode never constructs or renders that URL, preventing favicon identity disclosure.
