...
Review date: 28 July 2026. Target: Asterfold 3.0.1 on `agent/final-store-readiness-v3.0.1`.
...
The 3.0.1 manifest requests `activeTab`, `favicon`, `alarms`, `contextMenus` and `storage`, with optional `bookmarks`. `storage` is used only for `chrome.storage.session`, which keeps temporary Privacy Mode consistent between New Tab and popup and is cleared by Chrome at the end of the browser session. There are no host permissions or content scripts.
