# Chrome Web Store Privacy Practices — Asterfold 3.5.3

Prepared for the Asterfold 3.5.3 release. These are recommended dashboard answers for the reviewed default release. Recheck every answer against the final uploaded ZIP before submission.
...
The owner can certify the following for the reviewed 3.5.3 default build:
...
Asterfold 3.5.3 does NOT request `favicon`, `activeTab`, `alarms`, or `contextMenus`. The final 3.5.3 manifest must list only `permissions: ["storage"]` and optional `bookmarks`; it must not list host permissions, content scripts, `tabs`, `history`, `identity`, `scripting`, `webRequest`, cookies or clipboard-read. Because `favicon` is removed, the Chrome Web Store Developer Dashboard requires 0 permission justifications. If the final ZIP differs, stop submission and reconcile the manifest and disclosures.
