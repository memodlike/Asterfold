# Chrome Web Store submission checklist — Asterfold 3.6.2
...
- [ ] Policy version is 3.6.2 and effective date is 26 September 2026.
...
- [ ] Uploaded ZIP has `manifest_version: 3` and version `3.6.2`.
...
- [ ] `favicon` is used only to display Chrome-provided site icons for saved bookmarks via Chrome's browser-owned `/_favicon/` API; no external favicon services or host permissions.
