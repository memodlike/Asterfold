# Chrome Web Store submission checklist — Asterfold 3.5.3
...
- [ ] Policy version is 3.5.3 and effective date is 17 September 2026.
...
- [ ] Uploaded ZIP has `manifest_version: 3` and version `3.5.3`.
- [ ] Background runs as a service worker.
- [ ] New-tab override, popup and all required icons are present.
- [ ] Required permissions are strictly `storage`.
- [ ] `favicon` is not requested; icons render via local SVG vector fallbacks without remote or extension URL requests.
