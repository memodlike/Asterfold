# Install Asterfold 3.1.4 test candidate

> Asterfold 3.1.4 is an unpublished test candidate. The latest public stable version remains 3.1.2 until the required physical Windows 11 / Radeon R5 230 verification is accepted. Do not treat a successful CI run as Chrome Web Store publication.
...
4. Extract `Asterfold-Chrome.zip` into a permanent folder, for example `Documents/Asterfold-3.1.4-test`.
5. Open `chrome://extensions`.
6. Turn on **Developer mode**.
7. Click **Load unpacked**.
8. Select the extracted folder that directly contains `manifest.json`.
9. Verify that Chrome shows Asterfold version `3.1.4` and no extension errors.
...
4. Replace the existing unpacked folder contents with the extracted 3.1.4 runtime contents.
5. Open `chrome://extensions` and click **Reload** on Asterfold.
6. Open a new tab and verify version `3.1.4`, existing Pages, Boards, Bookmarks, wallpaper, locale, privacy state and Quick Save destinations.
7. Review Diagnostics and `chrome://extensions` for errors.

Schema migrations run transactionally. Automated CI verifies the exact 2.2.3 → 3.1.4 profile upgrade, but this does not replace the required physical hardware test.
...
- New settings introduced by 3.1.4 may be ignored by an older version; do not assume backward migration of the database schema.
...
- [Asterfold 3.1.4 physical test plan](./3.1.4-test-plan.md)
