      ...
      <div className="settings-layout">
        ...
        <div className="settings-content">
          ...
          {section === "language" ? <SettingsSection title={t("settings.language")} description={t("settings.languageDescription")}>
            <div className="language-options">{([{ value: "auto", label: t("settings.languageAuto") }, { value: "ru", label: t("settings.russian") }, { value: "kk", label: t("settings.kazakh") }] as const).map((item) => <button key={item.value} className={settings.locale === item.value ? "is-active" : ""} onClick={() => void patchSettings({ locale: item.value })}><span>{item.label}</span>{settings.locale === item.value ? <Check size={17} /> : null}</button>)}</div>
          </SettingsSection> : null}
