<div align="center">
  ...
  <p>Локально · Без регистрации · Русский и қазақша · Manifest V3</p>
</div>
...
Asterfold 2.0.3 — рабочее local-first расширение после полного редизайна. Оно не требует аккаунта, сохраняет эффект стекла по умолчанию и даёт ручной режим для слабых компьютеров через настройки.
...
1. **v2.0.3 — меню и ручная производительность.** Контекстное меню больше не прячется под блоками и не обрезается у края экрана. Эффект стекла снова включён по умолчанию, а режим для слабого ПК и анимации настраиваются вручную.
2. **v2.0.2 — лёгкий режим.** Добавлен режим, который уменьшает нагрузку на старые ПК и встроенную графику.
3. **v2.0.1 — правильная установка в Chrome.** Исправлен архив для Windows и Chrome: после распаковки `manifest.json` находится прямо в выбранной папке.
4. **v2.0.0 — большой редизайн.** Новая вкладка, блоки, локальное хранение, настройки, поиск, корзина и русский/қазақша интерфейс.
