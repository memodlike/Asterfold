<div align="center">
  ...
  <p>Локально · Без регистрации · 12 языков · Manifest V3</p>
</div>
...
| 12 языков | Автоопределение языка Chrome и ручной выбор в настройках |
...
Asterfold 2.1.3 — рабочее local-first расширение после полного редизайна. Оно не требует аккаунта, открывает обычные закладки в текущей вкладке и подстраивает интерфейс под язык Chrome.
...
1. **v2.1.3 — полный язык интерфейса.** Ошибки, уведомления, импорт, Quick Save, подсказки и подписи доступности теперь проходят через единый словарь. Настройки красиво переносят длинные строки на всех 12 языках.
2. **v2.1.2 — Search и Trash.** Поиск и корзина получили единый стеклянный дизайн: понятные фильтры, иконки, компактные действия, корректные пустые состояния и адаптацию к светлой, тёмной и пользовательской теме.
3. **v2.1.1 — контраст и вкладка Chrome.** Контекстное меню теперь получает цвета активной темы, остаётся полностью непрозрачным и открывается поверх интерфейса. Синяя иконка расширения в новой вкладке заменена на нейтральный серый значок документа, который адаптируется к светлой и тёмной теме Chrome.
4. **v2.1.0 — языки и переходы.** Добавлены английский, испанский и популярные европейские языки. В режиме «Авто» заголовок и интерфейс используют язык Chrome. Обычная закладка теперь открывается в этой же вкладке.
5. **v2.0.3 — меню и ручная производительность.** Контекстное меню больше не прячется под блоками и не обрезается у края экрана. Эффект стекла снова включён по умолчанию, а режим для слабого ПК и анимации настраиваются вручную.
6. **v2.0.2 — лёгкий режим.** Добавлен режим, который уменьшает нагрузку на старые ПК и встроенную графику.
7. **v2.0.1 — правильная установка в Chrome.** Исправлен архив для Windows и Chrome: после распаковки `manifest.json` находится прямо в выбранной папке.
8. **v2.0.0 — большой редизайн.** Новая вкладка, блоки, локальное хранение, настройки, поиск, корзина и русский/қазақша интерфейс.
