...
Asterfold 2.1.0 — рабочее local-first расширение после полного редизайна. Оно не требует аккаунта, открывает обычные закладки в текущей вкладке и подстраивает интерфейс под язык Chrome.
...
1. **v2.1.0 — языки и переходы.** Добавлены английский, испанский и популярные европейские языки. В режиме «Авто» заголовок и интерфейс используют язык Chrome. Обычная закладка теперь открывается в этой же вкладке.
2. **v2.0.3 — меню и ручная производительность.** Контекстное меню больше не прячется под блоками и не обрезается у края экрана. Эффект стекла снова включён по умолчанию, а режим для слабого ПК и анимации настраиваются вручную.
3. **v2.0.2 — лёгкий режим.** Добавлен режим, который уменьшает нагрузку на старые ПК и встроенную графику.
4. **v2.0.1 — правильная установка в Chrome.** Исправлен архив для Windows и Chrome: после распаковки `manifest.json` находится прямо в выбранной папке.
5. **v2.0.0 — большой редизайн.** Новая вкладка, блоки, локальное хранение, настройки, поиск, корзина и русский/қазақша интерфейс.
