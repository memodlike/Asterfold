...
test.describe.serial("Asterfold MV3 release", () => {
  ...
  test("shares session Privacy Mode with the popup and displays offline workspace dashboard", async () => {
    ...
    await expect(privatePopup.locator(".tab-preview small").first()).toHaveText("••••••");
    await expect(privatePopup.getByText("Приватный режим включён", { exact: true })).toBeVisible();
    ...
    await expect(popup.getByText("Asterfold 3.5.4")).toBeVisible();
    await popup.close();
