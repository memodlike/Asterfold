# Install Asterfold 1.0.0

## Requirements

- Chrome or a Chromium browser compatible with Chrome 120+.
- The extracted `release/chrome-unpacked` directory. No account or API key is required.
...
4. Select the `release/chrome-unpacked` directory itself, not its ZIP file.
5. Open a new tab. The Asterfold onboarding screen should appear.
6. Pin Asterfold from the extensions menu to keep Quick Save available.
7. Optionally open `chrome://extensions/shortcuts` and set the **Save the current page to Asterfold** shortcut. The suggested binding is `Ctrl+Shift+Y` (`Command+Shift+Y` on macOS).
...
5. Open a new tab and verify the version and Diagnostics. Schema migrations run transactionally; the current release upgrades schema 1 to schema 2 without clearing existing data.
